...
const whoIsBigger = function (n1, n2) {
  if (n1 < n2) {
    return "Il maggiore è " + n2;
  } else if (n1 > n2) {
    return "Il maggiore è " + n1;
  } else {
    return "I numeri sono uguali";
  }
};
console.log(whoIsBigger(20, 20));
...
console.log(splitMe("Ciao sono Felice"));
...
console.log(deleteOne("Ciao sono Felice", false));
...
const howManyDays = function (data) {
  ...
  const differenceDay = today - checkDate;
  const days = differenceDay / 86400000;
  return days;
};
console.log(howManyDays(2005 - 2 - 20));
/* ESERCIZIO 10
...
const newestMovie = function () {
  let newest = movies[0];
  movies.forEach((i) => {
    if (parseInt(i.Year) > parseInt(newest.Year)) {
      newest = i;
    }
  });
  return newest;
...
const onlyInLastMillennium = function () {
  return movies.filter((i) => parseInt(i.Year) < 2000);
};
...
const sumAllTheYears = function () {
  ...
  for (let i = 0; i < movies.length; i++) {
    const num = parseInt(movies[i].Year);
    sum = sum + num;
  }
  return sum;
};
...
const searchByTitle = function (s) {
  return movies.filter((m) => m.Title.toLowerCase().includes(s.toLowerCase()));
};
...
const searchAndDivide = function (s) {
  let searched = { match: [], unmatch: [] };
  for (let i = 0; i < movies.length; i++) {
    if (movies[i].Title.toLowerCase().includes(s.toLowerCase())) {
      searched.match.push(movies[i]);
    } else {
      searched.unmatch.push(movies[i]);
    }
  }
  return searched;
};
...
console.log(searchId("container"));
/* ESERCIZIO 21
...
*/
const redBckg = function () {
  const link = document.querySelectorAll("a");
  link.forEach((l) => (l.style.backgroundColor = "red"));
};
...
*/
const css = function () {
  const tag = document.querySelectorAll("tr");
  tag.forEach((t) => t.classList.add("test"));
};
// [EXTRA] JS Avanzato
